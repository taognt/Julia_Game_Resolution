...
function displaySolution(y::Matrix{VariableRef})
    y_int = Matrix{Int64}(undef, size(y, 1), size(y, 2))
    for i = 1:size(y, 1), j = 1:size(y, 2)
        y_int[i,j] = Int(value(y[i,j]))
    end
    n = size(y, 1)
...
"""
Pour l'heuristique :
PileCells est un tableau de tuples, chaque tuple étant les coordonnées d'une case grisée
"""
...
function writeSolution(fout::IOStream, y::Matrix{VariableRef})
